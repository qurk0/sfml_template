...
void Shape::update()
{
    ...
    m_vertices[0].position = Vector2f(m_insideBounds.left, m_insideBounds.top) +
                             Vector2f(m_insideBounds.width, m_insideBounds.height) / 2.f;
...
void Shape::updateTexCoords()
{
    ...
    for (std::size_t i = 0; i < m_vertices.getVertexCount(); ++i)
    {
        ...
        const float yratio      = m_insideBounds.height > 0
                                      ...
                                      : 0;
        m_vertices[i].texCoords = {convertedTextureRect.left + convertedTextureRect.width * xratio,
